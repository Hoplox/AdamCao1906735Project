...
class TFRBertUtil(object):
  ...
  def convert_to_elwc(self, context, examples, labels, label_name):
    ...
    for example, label in zip(examples, labels):
      ...
      """
      feature = {
        "queryText": tf.io.RaggedFeature(dtype=tf.string),
        "docText": tf.io.RaggedFeature(dtype=tf.string),
        "relevance": tf.io.FixedLenFeature(shape=(1,), dtype=tf.int64, default_value=0)
      }
      """
